...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="rounded-lg px-3 py-2 text-xs" style={{ background: "var(--secondary)", border: "1px solid var(--border)" }}>
      <div className="font-medium mb-1.5" style={{ color: "var(--foreground)" }}>{label}</div>
      {payload.map((p, i) => (
        <div key={i} className="flex items-center gap-2 mb-0.5">
          <span className="w-2 h-2 rounded-full" style={{ background: p.color }} />
          <span style={{ color: "var(--muted-foreground)" }}>{p.name}</span>
          <span className="font-medium" style={{ color: p.color, fontFamily: "'DM Mono', monospace" }}>{p.value}</span>
        </div>
      ))}
    </div>
  );
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: "Dispatched", value: dispatchedCount, color: "#f59e0b" },
          { label: "Resolved",   value: resolvedToday,  color: "#10b981" },
        ].map((kpi) => (
          <div key={kpi.label} className="rounded-lg p-3 text-center" style={{ background: "var(--secondary)", border: "1px solid var(--border)" }}>
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={110}>
          <BarChart data={hourlyViolations} margin={{ top: 0, right: 0, left: -24, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 6" stroke="var(--border)" />
            <XAxis dataKey="hour" tick={{ fill: "var(--muted-foreground)", fontSize: 9, fontFamily: "'DM Mono', monospace" }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: "var(--muted-foreground)", fontSize: 9, fontFamily: "'DM Mono', monospace" }} axisLine={false} tickLine={false} />
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={80}>
          <LineChart data={weeklyTrend} margin={{ top: 0, right: 0, left: -24, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 6" stroke="var(--border)" />
            <XAxis dataKey="day" tick={{ fill: "var(--muted-foreground)", fontSize: 9, fontFamily: "'DM Mono', monospace" }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: "var(--muted-foreground)", fontSize: 9, fontFamily: "'DM Mono', monospace" }} axisLine={false} tickLine={false} />
            <Tooltip content={<CustomTooltip />} />
            <Line type="monotone" dataKey="violations" stroke="#f59e0b" strokeWidth={1.5} dot={{ fill: "#f59e0b", r: 2.5 }} name="Total" />
          </LineChart>
        </ResponsiveContainer>
      </div>
        ...
        <ResponsiveContainer width="100%" height={130}>
          <PieChart>
            ...
            <Legend
              ...
              formatter={(val) => <span style={{ fontSize: 10, color: "var(--muted-foreground)" }}>{val}</span>}
              iconSize={7}
            />
            <Tooltip content={<CustomTooltip />} />
          </PieChart>
